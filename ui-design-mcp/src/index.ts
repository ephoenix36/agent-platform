...
server.tool(
  "generate_ui_design",
  "Generate an AI-powered UI design concept based on specifications",
  generateUIDesignSchema.shape,
  async (input: z.infer<typeof generateUIDesignSchema>) => {
    try {
...
- No watermarks or mockup frames, just the pure UI design`;
      ...
      if (!response.generatedImages || response.generatedImages.length === 0 || !response.generatedImages[0]?.image?.imageBytes) {
        return {
          content: [{ type: "text" as const, text: "Failed to generate UI design image" }],
          isError: true
        };
      }
...
server.tool(
  "generate_ui_variation",
  "Generate a variation of an existing UI design",
  generateVariationSchema.shape,
  async (input: z.infer<typeof generateVariationSchema>) => {
    try {
      // Read base image
      ...
      if (!generatedImage || !imageData) {
        return {
          content: [{ type: "text" as const, text: "Failed to generate variation" }],
          isError: true
        };
      }
      ...
      return {
        content: [
          { type: "text" as const, text: `✅ Generated UI variation: ${input.variation}` },
...
server.tool(
  "list_design_options",
  "List all available UI styles, color schemes, and component types",
  {},
  async () => {
    const options = {
...
server.tool(
  ...
  "Generate a complete design system with multiple components",
  generateDesignSystemSchema.shape,
  async (input: z.infer<typeof generateDesignSystemSchema>) => {
    try {
